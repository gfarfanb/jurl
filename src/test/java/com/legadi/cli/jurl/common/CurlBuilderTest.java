...
public class CurlBuilderTest extends EmbeddedAPIAbstractTest {
    ...
    @Test
    public void buildPostBinary() throws IOException, URISyntaxException {
        CurlBuilder curlBuilder = new CurlBuilder()
            .setUrl(new URI("http://localhost:" + port + "/basic/body").toURL())
            .setMethod("POST")
            .setDataBinary(Paths.get("src/test/resources/basic-functions.body.json"))
            .addHeader("Content-Type", "application/json");
        String expected = "curl -X POST"
            + " -H \"Content-Type: application/json\""
            + " --data-binary \"@" + toSystemSeparator("src/test/resources/basic-functions.body.json") + "\""
    ...
    @Test
    public void buildWithoutMethod() throws IOException, URISyntaxException {
        CurlBuilder curlBuilder = new CurlBuilder()
            .setUrl(new URI("http://localhost:" + port + "/basic/body").toURL());
    ...
    @Test
    public void buildPostFileWithoutName() throws IOException, URISyntaxException {
        CurlBuilder curlBuilder = new CurlBuilder()
            .setUrl(new URI("http://localhost:" + port + "/file").toURL())
            .setMethod("POST")
